const allCategories = [
    "Prénom","Pays","Ville","Animal","Objet","Métier","Marque","Fruit","Légume","Sport",
...
function renderCategorySelection() {
    ...
    container.innerHTML = "<h4>Choisir les catégories :</h4>";
...
function updateUsedLettersDisplay() {
    document.getElementById("usedLettersDisplay").innerHTML =
        "Lettres utilisées : " + usedLetters.join(" - ");
}

function closeAllRounds() {
    document.querySelectorAll(".round").forEach(r => r.classList.remove("open"));
}
...
function finishGame() {
    ...
    let average = (totalScore / roundData.length).toFixed(2);
    let bestRound = Math.max(...roundData.map(r => r.score));
    ...
    document.getElementById("statistics").innerHTML = `
        <div class="stats">
            ...
            <p>Moyenne par manche : ${average}</p>
            <p>Meilleure manche : ${bestRound}</p>
        </div>
    `;
}